...
public class LoginController {
	...
	public Result<String> sms(@RequestBody JSONObject jsonObject) {
		...
		try {
			...
			if (CommonConstant.SMS_TPL_TYPE_1.equals(smsmode)) {
				...
				obj.put("minute","10");
				b = QCloudSmsHelper.sendSms(mobile, obj, DySmsEnum.REGISTER_TEMPLATE_CODE);
			}else {
				//登录模式，校验用户有效性
				SysUser sysUser = sysUserService.getUserByPhone(mobile);
				result = sysUserService.checkUserIsEffective(sysUser);
				...
				if (CommonConstant.SMS_TPL_TYPE_0.equals(smsmode)) {
					//登录模板
					b = QCloudSmsHelper.sendSms(mobile, obj, DySmsEnum.LOGIN_TEMPLATE_CODE);
				} else if(CommonConstant.SMS_TPL_TYPE_2.equals(smsmode)) {
					//忘记密码模板
					b = QCloudSmsHelper.sendSms(mobile, obj, DySmsEnum.FORGET_PASSWORD_TEMPLATE_CODE);
				}
			}
	...
	private Result<JSONObject> userInfo(SysUser sysUser, Result<JSONObject> result) {
		...
		if (departs == null || departs.size() == 0) {
			...
			sysUserService.updateUserDepart(username,"A04A01"); // 新增用户为临时部门
		} else if (departs.size() == 1) {
			sysUserService.updateUserDepart(username, departs.get(0).getOrgCode());

			String roleId = "ee8626f80f7c2619917b6236f3a7f02b";//添加角色为临时角色
			SysUserRole sysUserRole = new SysUserRole(sysUser.getId(),roleId);
			QueryWrapper<SysUserRole> queryWrapper = new QueryWrapper<SysUserRole>();
			queryWrapper.eq("role_id", roleId).eq("user_id",sysUser.getId());
